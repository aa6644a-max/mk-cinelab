-- INSERT 트리거 수정: source를 'review:{id}' 형태로 저장해 삭제 시 추적 가능하게
...
-- 리뷰 삭제 시 XP 회수 트리거 함수
CREATE OR REPLACE FUNCTION revoke_review_xp() RETURNS TRIGGER AS $$
DECLARE
  v_xp INTEGER;
BEGIN
...
CREATE TRIGGER on_review_delete
  ...
  FOR EACH ROW EXECUTE FUNCTION revoke_review_xp();
